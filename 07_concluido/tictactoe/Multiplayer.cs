namespace tictactoe
{
    public partial class Multiplayer : Form
    {
        ...
        private void playerClick(object sender, EventArgs e)
        {
            ...
            else if (atual == Player.O)
            {
                vez.Text = "Vez do Player 1";
                var button = (Button)sender; // find which button was clicked
                currentPlayer = (Player.O); // set the player to X
                button.Text = currentPlayer.ToString(); // change the button text to player X
                button.Enabled = false; // disable the button
                button.BackColor = System.Drawing.Color.DarkSalmon; // change the player colour to Cyan
                buttons.Remove(button); //remove the button from the list buttons so the AI can't click it either
                atual = Player.X;
                Check(); // check if the player won
            }
        ...
        private void Check()
        {
               ...
               || button3.Text == "X" && button5.Text == "X" && button7.Text == "X")
            {
                ...
                MessageBox.Show("Player 1 Wins"); // show a message to the player
                playerWins++; // increase the player wins 
                label1.Text = "Player 1 Wins- " + playerWins; // update player label
            ...
            || button3.Text == "O" && button5.Text == "O" && button7.Text == "O")
            {
                ...
                MessageBox.Show("Player 2 Wins"); // show a message box to say computer won
                computerWins++; // increase the computer wins score number
                label2.Text = "Player 2 Wins- " + computerWins; // update the label 2 for computer wins
